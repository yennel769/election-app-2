#!/usr/bin/env python3
"""
app-rebooter2.py — SECONDARY watchdog for app_ui (UI) + peer babysitter (rebooter1).
- Exposes its own /health (default :9046)
- Checks peer /health (default :9047) and restarts rebooter1 if it's down
...
Env overrides (optional):
  ...
  R2_HEALTH_PORT=9046
  R1_HEALTH_URL=http://127.0.0.1:9047/health
...
- Gunicorn is launched with access/error logs to stdout ("-"), critical level, captured,
  and Popen pipes that to DEVNULL; no files or folders are created.
"""
...
R2_HEALTH_PORT = int(os.getenv("R2_HEALTH_PORT", "7046"))
R1_HEALTH_URL  = os.getenv("R1_HEALTH_URL", "http://127.0.0.1:7047/health")

# Peer (PRIMARY) launch details
PEER_FILE    = "app-reboot-ui1.py"
PEER_PIDFILE = "reboot-ui1.pid"
...
def shell(cmd: str):
    # still useful internally for clears; output captured but we never print
...
def start_peer_inline():
    """
    Start app-reboot-ui1.py silently; write only reboot-ui1.pid. No log dirs/files.
    """
...
class _HealthHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt, *args):
        return  # silence BaseHTTPRequestHandler logging
...
def main():
    ...
    while True:
        ...
        # keep UI alive (with defer so primary gets first crack)
        if not ui_health_ok():

#!/usr/bin/env python3
"""
app-rebooter2.py — SECONDARY watchdog for app_hub + peer babysitter (rebooter1), with **no logs**.
- Exposes its own /health on R2_HEALTH_PORT (default :9049)
- Checks peer /health (default http://127.0.0.1:9050/health) and restarts rebooter1 if it's down
...
R2_HEALTH_PORT = int(os.getenv("R2_HEALTH_PORT", "7049"))
R1_HEALTH_URL  = os.getenv("R1_HEALTH_URL", "http://127.0.0.1:7050/health")
...
def start_peer_inline():
    """
    Start app-rebooter1.py silently; write only reboot1.pid. **No log dirs/files.**
    """
